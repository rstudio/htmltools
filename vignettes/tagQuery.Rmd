...
Notice how `tagQuery()` tracks two essential pieces: the _root_ (i.e., input) tag as well as _selected_ tags (by default the root is selected). This data structure allows us to [efficiently](#performance) [query](#query) and [modify](#modify) particular fragments of the root HTML tag.
...
To reset the set of selected tags to the root tag, use `$reset()`:
