...
Performs like a standard stack until it hits 500 elements. At 500 elements, the internal stack is converted to a `uni_stack()`.

While there is overhead at the 500 mark, the time penalty should be recovered as `n` becomes larger as it will behave like a `uni_stack()`.
...
hybrid_stack <- function() {
  ...
  list(
    ...
    as_list = function() {stack$as_list()}
  )
}
```
...
info <-
  lapply(1:10, function(i) {
    lapply(
      c(10,50,100, 250,500, 750,1000,5000),
      function(n) {
        cat("i: ", i, "; n: ", n, "\n", sep = "")
        ...
        ret <-
          bench::mark(
            ...
            populate(uni_stack())$as_list(),
            populate(hybrid_stack())$as_list()
          )
        ret$n <- n
        ret$i <- i
        ret
